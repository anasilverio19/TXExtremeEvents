...
#Call in environmental data from depth
#table.1excel <- read.csv("~/texas_extreme_events/Table1_Excel.csv")
#enviro_join <- read.csv("~/texas_extreme_events/enviro_join.csv")

#merge data
#enviro_join <- enviro_join %>% 
  #rename("MajorBay" = "MAJOR_AREA_CODE")

#table1 <- merge(table.1excel,enviro_join)



#table1 <- table1 %>% 
  #rename("Major Bay" = "MajorBay") %>% 
...
#table1$`Major Bay` <- factor(table1$`Major Bay` , levels = c("SabineLake", "GalvestonBay", "MatagordaBay", "SanAntonioBay", "AransasBay", "CorpusChristiBay", "UpperLagunaMadre", "LowerLagunaMadre"))
...
## looks like for red drum, there are lulls and peaks and the most consistence peaks are sept-april of every year, lowest during the summer
...
```{r recruitment windows closer look, include=FALSE}
...
## In regards to suspecting an already declining population, being able to compare to an averaged percent change, if the change is more negative than that, then we could say that the storm was harmful to an already declining populations = vulnerable
...
```{r plotting all together hurricane}
## All the spread per change into one dataframe?

check1 <- rbind(RE.RD.Spread.PerChange, RE.SST.Spread.PerChange, RE.BD.Spread.PerChange,RE.SF.Spread.PerChange)

#copy over to a new dataframe: 
plot <- check1
...
#gather:
plot <- plot %>% pivot_longer(cols = c('first_perChange', 'second_perChange'),
                                names_to = 'per_change',
...
#second try building the figure 



ggplot(plot, aes(x=MAJOR_AREA_CODE, y = percentage, group = SPECIES, color = SPECIES, shape = per_change)) + geom_point(position = position_dodge(width=1)) + theme_classic() + theme(axis.text.x = element_text(angle=90, hjust=1)) + geom_hline(yintercept = 0) 
...
I am putting a pin on the red drum for now given their unique recruitment window
